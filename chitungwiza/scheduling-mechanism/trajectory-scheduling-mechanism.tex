...
\caption{\textbf{The residue floor and its information capacity.} The
diagonal floor $\floor_A$ is strictly positive and tracks $1/\abs{K_A}$;
no candidate is read below it; and the number of distinguishable residue
values on $[\floor_A,U]$ obeys the predicted $\lceil(U-\floor_A)/\eps\rceil$
bound (\Cref{thm:floor,thm:info}).}
\label{fig:panel1}
\end{figure*}
...
\caption{\textbf{Strict monotonicity of the trajectory count.} The count
increments by exactly one per work unit and never decrements, even when a
fraction of units are ``undo'' operations restoring a prior candidate;
revisited candidates yield distinct $(x,\Mcount)$ states
(\Cref{thm:monotone}, \Cref{cor:noreturn}).}
\label{fig:panel2}
\end{figure*}
...
\caption{\textbf{Trajectory inflation and logarithmic termination.} The
closed form $T(n,d)=d(d+1)^{n-1}$ matches direct enumeration exactly; the
content grows exponentially in committed units while cost grows linearly;
and $n_{\mathrm{term}}(K,d)$ is logarithmic in $K$
(\Cref{thm:inflation,thm:termination}).}
\label{fig:panel3}
\end{figure*}
...
\caption{\textbf{Runtime diagnosis: compute- vs.\ structure-limited.} A
geometrically descending residue stream ($\Delta>0$) is read as
compute-limited; a stream that plateaus above the floor ($\Delta=0$) as
structure-limited; the classification uses the residue stream alone
(\Cref{thm:diagnosis}).}
\label{fig:panel4}
\end{figure*}
...
\caption{\textbf{Residue priority and scheduler soundness.} Priority
rises with descent rate and with closeness to threshold, is zero for
stalled tasks and unbounded at threshold; over random task sets a stalled
task never out-prioritises a converging one (\Cref{def:priority},
\Cref{thm:sound}).}
\label{fig:panel5}
\end{figure*}
...
\caption{\textbf{Sufficiency stopping.} A sub-task released at the parent
threshold $\theta$---above its own floor---leaves the global attainable
residue unchanged while saving the committed cost of the additional units;
the saved cost grows as the parent floor coarsens relative to the
sub-task floor (\Cref{thm:sufficiency}).}
\label{fig:panel6}
\end{figure*}
...
\caption{\textbf{Avalanche and time-blindness of a content digest.} A
single input-bit flip induces a near-half (fraction $\approx0.5$) output
bit difference, so related inputs receive maximally distant digests; and
identical content at distinct times yields identical digests
(\Cref{thm:orthogonality,thm:timeblind}).}
\label{fig:panel7}
\end{figure*}
...
\caption{\textbf{Prefix is proximity; the paired label.} The
longest-common-prefix of two trajectory addresses equals their shared
subtree depth; addresses extend monotonically without collision; and the
paired content--time label separates recurrence, co-temporality, and
refinement (\Cref{thm:prefix,thm:queries}).}
\label{fig:panel8}
\end{figure*}
