"""
ADVANCED STATISTICAL VISUALIZATION FOR PHASE-LOCKED FOLDING
...
if __name__ == "__main__":
    ...
    critical_indices = np.where(np.diff(stability) > 0.1)[0]
    ...
    curvature = np.abs(np.diff(np.arctan2(dv, ds)))
    ...
    TRAJECTORY SHAPE:
    Mean curvature: {np.mean(curvature):.4f} rad
    Max curvature: {np.max(curvature):.4f} rad
    ...
    Volume explored: {np.ptp(stability) * np.ptp(variance) * np.ptp(phase_coherence):.4f}
    Density: {len(cycles) / (np.ptp(stability) * np.ptp(variance) * np.ptp(phase_coherence)):.1f} points/unit³
    """
    ...
    stability_hist = stability_hist / np.histogram(cycle_phase, bins=bin_edges)[0]
    ...
    # Panel 6E: Rose Diagram (Directional Histogram)
    ...
    variance_circle = plt.Circle((0, 0), circular_variance, transform=ax6f.transData._b,
                                fill=False, edgecolor='blue', linewidth=3, linestyle='--')
    ax6f.add_artist(variance_circle)
    ...
    # Treat each cycle as an oscillator with phase = cycle_phase
    # and amplitude = stability

    order_parameter = []
    for i in range(len(cycles)):
        # Calculate order parameter up to cycle i
    ...
    Interpretation: {'High concentration' if mean_resultant > 0.7 else 'Moderate concentration' if mean_resultant > 0.3 else 'Low concentration'}

    UNIFORMITY TEST (Rayleigh):
    Test statistic (z): {z:.4f}
    p-value: {p_rayleigh:.6f}
    Result: {'REJECT uniformity (p<0.05)' if p_rayleigh < 0.05 else 'Cannot reject uniformity'}
    Interpretation: {'Data shows significant directionality' if p_rayleigh < 0.05 else 'Data may be uniform'}

    SYNCHRONIZATION:
    Final order parameter: {order_parameter[-1]:.4f}
    Synchronization achieved: {'YES (r > 0.7)' if order_parameter[-1] > 0.7 else 'PARTIAL (0.3 < r < 0.7)' if order_parameter[-1] > 0.3 else 'NO (r < 0.3)'}
    Cycles to sync: {np.where(order_parameter > 0.7)[0][0] + 1 if np.any(order_parameter > 0.7) else 'N/A'}

    DIRECTIONAL STATISTICS:
    Preferred direction: {np.degrees(circular_mean):.1f}° (cycle phase)
    Direction variance: {circular_variance:.4f}
    Concentration parameter (κ): {1/(circular_variance + 1e-10):.2f}

    INTERPRETATION:
    {'✓ Strong phase-locking observed' if mean_resultant > 0.7 else '⧗ Moderate phase-locking' if mean_resultant > 0.3 else '✗ Weak phase-locking'}
    {'✓ Significant synchronization' if order_parameter[-1] > 0.7 else '⧗ Partial synchronization' if order_parameter[-1] > 0.3 else '✗ No synchronization'}
    {'✓ Non-uniform distribution (directional)' if p_rayleigh < 0.05 else '⧗ Possibly uniform distribution'}
    """
    ...
    ax6i.plot(cycles[1:], angular_velocity, 'o-', linewidth=2.5, markersize=8,
            color='#e74c3c', alpha=0.8)
    ...
    # Create vector field showing flow in polar coordinates
    theta_grid = np.linspace(0, 2*np.pi, 20)
    r_grid = np.linspace(0, 1, 10)
    THETA, R = np.meshgrid(theta_grid, r_grid)

    # Calculate "flow" based on actual data
    # Interpolate stability gradient
    ...
    for i in range(len(theta_grid)):
        for j in range(len(r_grid)):
            ...
            distances = np.sqrt((cycle_phase - THETA[j, i])**2 + (stability - R[j, i])**2)
            nearest_idx = np.argmin(distances)

            if nearest_idx < len(ds):
                # Use actual gradient
                U[j, i] = ds[nearest_idx] * 0.1
                V[j, i] = (cycle_phase[nearest_idx+1] - cycle_phase[nearest_idx]) * 0.1

    # Plot vector field
    ax6j.quiver(THETA, R, U, V, alpha=0.5, color='blue', scale=0.5)
    ...
    # Calculate phase sensitivity: derivative of PRC
    if fit_success:
    ...
    VARIANCE PRC:
    Amplitude: {popt_variance[1]:.4f}
    Phase shift: {popt_variance[2]:.4f} rad
    2nd harmonic: {popt_variance[3]:.4f}
    Mean: {popt_variance[0]:.4f}

    Peaks: {len(peaks_v)}
    Troughs: {len(troughs_v)}

    SENSITIVITY:
    Max: {np.max(phase_sensitivity):.4f}
    Min: {np.min(phase_sensitivity):.4f}
    Zero crossings: {len(zero_crossings)}

    INTERPRETATION:
    {'✓ Strong periodic response' if abs(popt_stability[1]) > 0.1 else '⧗ Weak periodic response'}
    {'✓ Phase-locked dynamics' if len(peaks) > 0 and len(troughs) > 0 else '⧗ No clear phase-locking'}
    """
    else:
        prc_stats_text = """
    PRC STATISTICS:
    ...
    # Add small perturbations and see how system responds
    ...
    for pert_phase in perturbation_phases:
        ...
        # Calculate response (change in stability after perturbation)
    ...
    print("\nNext: Wavelet analysis and Cosinor rhythmometry...")
    print("="*80)
