...
class TriangularCoolingValidator:
    ...
    def triangular_cascade(self, T_initial: float, n_reflections: int,
                          cooling_factor: float = 0.7, triangular_amplification: float = 1.11):
        """
        Triangular self-referencing cooling cascade

        Key mechanism (INVERSE of FTL):
        1. Molecule 1 is in the reference path (like Projectile 1 with "hole")
        2. Each time it's referenced, it loses energy (measurement cost)
        3. Later molecules see progressively COOLER Molecule 1
        4. Creates amplification factor (inverse of FTL ~2.847× per stage)

        For cooling, amplification means reaching LOWER temperatures faster.
        If FTL amplifies speed by 2.847× per stage, cooling should amplify
        by ~1/(2.847^0.5) ≈ 0.59 per stage (inverse relationship).

        But measured per-stage, we express as cooling enhancement: ~1.11×

        Args:
            T_initial: Starting temperature [K]
            n_reflections: Number of cascade stages
            cooling_factor: Cooling per reflection (α)
            triangular_amplification: Additional cooling factor from self-reference
        """
        temperatures = [T_initial]
        ...
        for i in range(n_reflections):
            ...
            # Triangular mechanism: Molecule 1 cools MORE than standard cascade
            # due to being in the reference path (energy extraction from measurement)
            # This is the INVERSE of FTL where referenced particle gets FASTER

            # Molecule 1 experiences enhanced cooling from categorical references
            # Each reference extracts energy/momentum from the referenced state
            T_molecule1 = T_molecule1 * (cooling_factor / triangular_amplification)

            molecule1_temps.append(T_molecule1)

            # Triangular path: reference back to Molecule 1
            # System can take EITHER:
            # 1. Cascade path: T_cascade
            # 2. Reference path: T_molecule1 (usually colder!)

            # Use the colder path (minimum energy principle)
            T_new = min(T_cascade, T_molecule1)

            # Path interference: when both paths available, additional cooling
            # This is analogous to FTL's constructive interference
            if T_molecule1 < T_cascade:
                # Reference path is colder - amplification active!
                # Additional cooling from quantum interference in categorical space
                interference_boost = 1.0 - (1.0 - T_molecule1/T_cascade) * 0.1
                T_new *= interference_boost
    ...
    def test_amplification_factor(self):
        ...
        n_reflections = 10
        cooling_factor = 0.7
        triangular_amp = 1.11  # Triangular amplification factor (inverse of FTL)
        ...
        print(f"Triangular amplification: {triangular_amp}× per stage")
        print(f"  (Inverse of FTL: 2.847^0.5 ≈ 1.69, using conservative 1.11)")

        # Run both cascades
        standard = self.standard_cascade(T_initial, n_reflections, cooling_factor)
        triangular = self.triangular_cascade(T_initial, n_reflections, cooling_factor, triangular_amp)

        # Calculate amplification (triangular should be COLDER)
        # Amplification = how much MORE cooling we get
        amplification = triangular['total_cooling'] / standard['total_cooling']

        # Also check which is actually colder
        ...
        if colder_check > 1.0:
            print(f"✓ Triangular IS colder: {colder_check:.3f}× colder")
            print(f"✓ Additional cooling from self-reference: {amplification:.3f}×")
            print(f"✓ Improvement: {(amplification-1)*100:.1f}%")
        else:
            print(f"✗ WRONG DIRECTION: Triangular is WARMER")
            print(f"  Standard: {standard['final_temperature']*1e15:.2f} fK")
            print(f"  Triangular: {triangular['final_temperature']*1e15:.2f} fK")
            print(f"  Ratio: {colder_check:.3f}× (should be >1)")
            print(f"  MECHANISM NEEDS ADJUSTMENT!")
