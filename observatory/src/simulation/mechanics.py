...
"""
prisoner_wave_mechanics.py

Maxwell's demon with full wave interference mechanics.
Categories emerge from interference patterns.
"""

import numpy as np
from scipy.fft import fft, fftfreq

class WaveBasedPrisoner:
    """
    Prisoner parable with wave mechanics.

    - Source wave = fundamental reality
    - Particles = interference generators
    - Demon = pattern recognizer
    - Categories = stable interference patterns
    """

    def __init__(self, omega_0=1e13):
        self.omega_0 = omega_0  # Source frequency
        self.source_wave = SourceWave(omega_0)
        self.interference_cascade = []
        self.categories = []

    def add_particle(self, particle):
        """Add particle, compute interference, extract categories"""
        # Compute interference
        pattern = self._compute_interference(particle)
        self.interference_cascade.append(pattern)

        # Extract categories from pattern
        cats = self._extract_categories(pattern)
        self.categories.extend(cats)

    def _compute_interference(self, particle):
        """Compute interference pattern from particle"""
        # Get current wave (source + all previous interference)
        current_wave = self._get_current_wave()

        # Particle scatters wave
        scattered = self._scatter_wave(current_wave, particle)

        # Total = current + scattered
        return current_wave + scattered

    def _extract_categories(self, pattern):
        """Extract stable patterns (categories) from interference"""
        # FFT to find dominant frequencies
        spectrum = fft(pattern)
        frequencies = fftfreq(len(pattern))

        # Find peaks = stable patterns = categories
        peaks = self._find_peaks(spectrum)

        # Each peak is a category
        categories = [Category(freq, amp) for freq, amp in peaks]
        return categories
