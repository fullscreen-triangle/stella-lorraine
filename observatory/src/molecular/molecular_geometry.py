...
if __name__ == "__main__":
    ...
    with open('public/vanillin_bonds.json', 'r') as f:
    ...
    ax10.set_xticklabels(labels_fc, rotation=45, ha='right', fontsize=8)
    ...
    # FIXED: Use tick_labels instead of labels
    bp = ax11.boxplot([aromatic_freqs, non_aromatic_freqs],
                    tick_labels=['Aromatic', 'Non-Aromatic'],
    ...
    bp = ax12.boxplot([conjugated_lengths, non_conjugated_lengths],
                    tick_labels=['Conjugated', 'Non-Conjugated'],
                    widths=0.6, patch_artist=True,
                    showmeans=True, meanline=True)
    ...
    # FIXED: Use np.median instead of .median()
    ax13.axvline(np.median(frequencies_thz), color='green', linestyle=':',
