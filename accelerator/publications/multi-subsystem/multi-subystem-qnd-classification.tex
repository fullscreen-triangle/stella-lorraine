...
\begin{remark}[Jackson independence in the universal transport primitive]
Complete commutation $[\hat{O}_n, \hat{O}_\ell] = \cdots = 0$ is
equivalent to \emph{diagonal coupling} in the universal transport
primitive \citep{companion:transport}: the coupling matrix $g^{(ij)}$
between decision classes is diagonal (no cross-class transitions carry
information between the commuting observables).
The Jackson-network throughput formula — Specialisation (2) of the
universal transport theorem \citep{companion:transport} — is the
computational realisation of the same fact: a network of independent
decision classes has product-form throughput, each class contributing
independently.

In the LHC context: the four detector subsystems (tracker, ECAL, HCAL,
muon) are the four decision classes, and diagonal coupling means their
dispatch can be scheduled in parallel by the kernel's PSS subsystem
\citep{companion:kernel} without contention. This is why the ATLAS L1
trigger can combine all four subsystem measurements within one 2.5~$\mu$s
latency window.
\end{remark}
