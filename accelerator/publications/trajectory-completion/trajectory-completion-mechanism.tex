...
\begin{theorem}[Selection Rules as Dead Zones]
\label{thm:selection_rules}
Trigger backgrounds that require selection-rule-violating intermediate
transitions between partition states cannot be reached by backward completion
from any physically realizable endpoint.
\end{theorem}
...
Formally: the backward algorithm uses only the parent map
$p_j \mapsto p_{j-1}$, which by definition respects the refinement order.
The refinement order imposes exactly the selection rules. Backgrounds
requiring forbidden transitions are not merely rare — they have no pre-image
under the parent map and thus have zero probability under any backward
completion from a physical endpoint.
\end{proof}
