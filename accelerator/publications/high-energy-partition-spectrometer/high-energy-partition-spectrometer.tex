...
\begin{remark}[Three-route verification at Planck depth]
Theorem~\ref{thm:mass_equiv} is an instance of the general Three-Route
Equivalence Verification Primitive \citep{companion:verification}: three
computationally independent routes to the same bounded categorical invariant
(the mass $M$) must agree to within a closed-form bound
$2K(d+1)^{-\nP}$.
The three routes correspond exactly to the three generic route types:
Route~I (residue) $=$ accumulated path length $M_{\mathrm{residue}}$;
Route~II (confinement) $=$ confinement-cost $M_{\mathrm{confinement}}$;
Route~III (negation fixed point) $=$ $M_{\mathrm{fixed-point}}$.
At LHC Planck depth $\nP = 60$ and $d = 3$: the disagreement bound is
$2K \cdot 4^{-60} \approx K\times 10^{-36}$, essentially exact for any
physical application.
The formal verification procedure of \citet{companion:verification} can
be run as a kernel-level conformance test: dispatching all three routes
in parallel and checking their agreement to within this bound is a
deterministic, $\mathcal{O}(1)$ check at LHC Planck depth.
\end{remark}
