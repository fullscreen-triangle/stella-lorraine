...
\begin{figure*}[!htbp]
    ...
    (\textbf{B})~Required bijection violations as a function of reversal depth $k$, for a
    reversal originating at $M^{*} = 100$: violations$(k) = k$ exactly.  A depth-$k$
    reversal forces $k$ distinct integers in $\Zp$ to share a single image under
    $\Phi$, contradicting Theorem~\ref{thm:bijection}.
    ...
    injective with no collisions.
    (\textbf{D})~Manhattan distance $d(\Phi(M^{*}),\,\Phi(M^{*}{-}k))$ in partition-label
    space for $M^{*} = 100$, $k = 1,\ldots,50$, coloured by $k$ (plasma scale).
    Every distance is strictly positive: no two distinct integers share a partition-state
    image, confirming that any claimed decrement would produce a genuine collision,
    not a near-miss.}%
...
\textbf{What Physics Actually Calls ``Time Reversal.''}
